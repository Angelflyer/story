...
(function (TD) {
  ...
  U.init = function () {
    ...
    TD.TOWER_ORDER.forEach((id, i) => {
      ...
      const img = document.createElement('img'); img.src = R3().towerIcon(id, 0, 128); img.alt = def.name; b.appendChild(img);
  ...
  U.onSelect = function (sel) {
    ...
    else { U.refreshTowerMenu(); const t = G().towers[sel.index]; const h = { archer: 90, ballista: 60, mage: 150, catapult: 50 }[t.type] + t.level * 10; U.anchor = { el: $('tmwrap'), x: p[0], z: p[1], h }; $('tmwrap').classList.remove('hidden'); }
    U.placeAnchor();
  };
  ...
  U.buildCredits = function () {
    ...
    b.innerHTML = `
      ...
      <p>Rendering: three.js (MIT). Fonts: Cinzel and Crimson Pro (SIL OFL) via Google Fonts. All 3D models are built procedurally by the game.</p>`;
  };
  TD.UI = U;
})(window.TD);
